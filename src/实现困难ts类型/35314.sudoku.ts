...
// type Revered = ReverseMatric<Matrix>;
...
C extends number = F,
R extends number[] = []> =
LessThan<Plus<C,1>&number,T> extends true ? BuildRange<F,T,Plus<C,1>&number,[...R,C]>: R[number]
...
> = FlattenObj<FlattenObj<{
    [r in BuildRange<X1, X2>]: Matric[r] extends infer ColumnResults extends number[]
...
// Y extends number = 0,
// R extends any[] = [],
// > = X extends Len ? Y extends Len ? R :
// X extends Len ? TransFormSubMatrix<Matric,Len,Y,Plus<Y,3>&number,[...R,
//     [ExtractBlock<Matric,X,Plus<X,3>&number,Y,Plus<Y,3>&number>]
// ]>:Y extends Len ? TransFormSubMatrix<Matric,Len,0,Plus<Y,3>&number,[...R,
//     [ExtractBlock<Matric,X,Plus<X,3>&number,Y,Plus<Y,3>&number>]
// ]>
